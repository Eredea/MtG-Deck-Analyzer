...
class LeftDisplay(tkinter.Frame):
    ...
    def __init__(self, parent, **kwargs):
        ...
        self.cardSearchBox = tkinter.ttk.Entry(self)
        self.cardSearchBox.insert(0, "Swords to plowshares")
        self.searchResultsList = DeckListDisplay(self, linkedDisplay=True)

        # Buttons
        self.searchButton = tkinter.ttk.Button(self, text="Search for card", command= lambda: self.searchResultsList.show_deck(mgu.search(name = self.cardSearchBox.get())))
        self.deckListButton = tkinter.ttk.Button(self, text="Add to DeckList")
        self.cardPoolButton = tkinter.ttk.Button(self, text="Add to Cardpool")
        self.advancedSearchButton = tkinter.ttk.Button(self, text = "Bring advanced search", command = lambda: AdvancedSearchWindow(self.master))
